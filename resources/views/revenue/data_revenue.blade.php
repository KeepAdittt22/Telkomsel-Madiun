...
@section('content')
    ...
    <div class="card">
        <div class="card-body">
            <button type="button" class="btn btn-warning" data-toggle="modal" data-target="#modal-tambah" onclick="clear_target()">
                ...
                <tbody>
                    @foreach ($revenue as $Revenue)
                        <tr>
                            ...
                            {{-- <td>{{  number_format($Revenue->MoM,"0",",",".")  }}</td>
                            <td>{{  number_format($Revenue->YoY,"0",",",".")}}</td>
                            <td>{{ number_format($Revenue->YtD,"0",",",".")}}</td> --}}
                            <td style="text-align: center">
                              <a href="javascript:void(0)" class="btn btn-warning btn-flat btn-sm" data-toggle="modal" data-target="#modal-tambah" onclick="edit_target('{{ $Revenue->id_revenue }}','{{ $Revenue->id_product }}','{{ $Revenue->jml_target }}','{{ $Revenue->jml_revenue_old }}','{{ $Revenue->jml_revenue_new }}','{{ $Revenue->MoM}}','{{ $Revenue->YoY }}','{{ $Revenue->YtD }}')"><i class="fa fa-edit"></i></a>
                              <a href="{{ url('revenue/delete/'.$Revenue->id_revenue) }}" class="btn btn-danger btn-flat btn-sm" onclick="return confirmDelete(this)"><i class="fa fa-trash"></i></a>
                            </td>
                        </tr>
                    @endforeach
                </tbody>
        ...
        <div class="modal-dialog modal-lg" style="background-color: beige">
            <div class="modal-content">
                <div class="modal-header bg-danger">
                    <h4 class="modal-title">Data Revenue Kecamatan</h4>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <form action="{{url('/revenue/save')}}" method="post" id="form_rev">
                    @csrf
                    <input type="hidden" id="id_revenue" name="id_revenue" value="">
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-4">
                                    <div class="form-group">
                                        ...
                                        <input type="number" step="0.000000001" class="form-control @error("MoM") is-invalid  @enderror" id="MoM" name="MoM" placeholder="%MoM">
                                        @error("MoM")
                                ...
                                <div class="col-md-4">
                                    <div class="form-group">
                                        ...
                                        <input type="number" step="0.000000001" class="form-control @error("YoY") is-invalid  @enderror" id="YoY" name="YoY" placeholder="%YoY">
                                        @error("YoY")
                                ...
                                <div class="col-md-4">
                                    <div class="form-group">
                                        ...
                                        <input type="number" step="0.000000001" class="form-control @error("YtD") is-invalid  @enderror" id="YtD" name="YtD" placeholder="%YtD">
                                        @error("YtD")
                                ...
                                <p style="color: red; margin-bottom: 0">*Saat merubah data MoM,YoY & YtD ambil 4 angka dibelakang koma dan bulatkan angka terakhir</p>
                            </div>
                        </div>
                    </div>
