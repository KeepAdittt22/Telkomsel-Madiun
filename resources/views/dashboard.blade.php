...
<div class="data-revenue-cluster" style="white-space:nowrap; overflow-x: scroll;
    ...
    <div style="display: inline-block">
        <table class="table">
            <thead>
                <tr class="bg" style="background-color:#f01d06; color: rgba(247, 239, 239, 0.932);">
                    <th colspan="7" class="text-center">{{ $revItem["title"] }}</th>
                </tr>
            <tr>
            ...
            <tbody>
                @foreach ($revItem["data"] as $revenue)
                    <tr>
                        ...
                        @endif
                        <td>{{ Helper::convert_number($revenue->jml_target) }}</td>
                        <td>{{  Helper::convert_number($revenue->jml_revenue_old,"0",",",".") }}</td>
                        <td>{{  Helper::convert_number($revenue->jml_revenue_new,"0",",",".") }}</td>
...
<div class="modal fade" id="modal-tambah" role="dialog">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <form action="{{url('/')}}" method="post" id="form_rev" onsubmit="showPreload()">
                @csrf
...
<script>
    $(function(){
        @if($errors->any())
        showMessage("Terjadi Kesalahan !","Data Gagal Diupdate","error","Oke");
